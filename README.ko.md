...
# XXD Panel 117｜가는 선 담채 여백 기록

하나의 시각 중심점으로 사진의 주제와 여백을 다시 연출합니다.
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 117 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 117 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 117 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 117 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 117 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 117 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 117 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 117 additional top-bottom sample 4"></td>
  </tr>
</table>
...
사진이 복잡하거나 구도가 평범해도 남길 가치가 있는 무언가가 있다면 **Panel 117**은 하나의 핵심 주제, 시각적 중심점, 감정적 관계를 찾아 가는 선, 옅은 평면 채색, 종이 질감, 가벼운 콜라주로 작은 화면을 다시 연출합니다. 비교 영역에 실제 사진을 남기면서 의도적인 여백으로 디자인의 독립성을 만듭니다.

### 이런 경우에 적합합니다

- 원물의 식별성을 지키면서 위치, 크기, 방향, 크롭을 다시 설계하고 싶을 때.
- 사물별 모사와 복잡한 배경 대신 적은 요소와 강한 주제가 필요할 때.
- 사실적인 세부나 매끈한 벡터보다 가는 선, 부드러운 파스텔, 숨 쉬는 종이를 원할 때.
- 상하, 좌우, 디자인 전용, 여러 비율, 4기기 배경화면, 폴더 일괄 출력이 필요할 때.

### 해결하는 것

- 복잡한 소재에서 핵심 시각 중심점을 선택하고 부차 정보를 삭제합니다.
- 여백이 양·음의 형태, 거리, 비대칭 균형을 구성하게 합니다.
- 비교는 정확히 50%씩 두 영역만 사용하고 제3영역이나 내부 패널을 만들지 않습니다.
- 각 결과를 현재 원본에서 독립적으로 한 번 생성해 중간 결과의 재전환을 방지합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-117`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-117 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-117-prompt.en.md)와 [중국어](references/xxd-panel-117-prompt.zh-CN.md)를 확인하세요.

## 원본 프롬프트 · 5개 언어

[简体中文](references/original-prompt/zh-CN.md) · [English](references/original-prompt/en.md) · [日本語](references/original-prompt/ja.md) · [한국어](references/original-prompt/ko.md) · [العربية](references/original-prompt/ar.md)

중국어 파일은 원문을 그대로 보존하며 런타임의 유일한 창작·미학 권위입니다. 다른 네 언어는 완전하고 충실한 열람용 번역입니다. 좌우 등 기본값 이외 모드를 명시적으로 선택하면 어댑터는 납품 위치와 크기만 바꾸며 미학은 유지합니다.

**핵심 특징:** 하나의 핵심 주제 · 하나의 시각 중심점 · 하나의 감정 관계 · 가는 손그림 선 · 옅은 평면 채색 · 종이 질감 · 가벼운 콜라주 · 작은 도장 규모 · 2–4색 · 넓은 여백 · 가는 손글씨

## 빠른 적합성 확인

| 궁금한 점 | Panel 117의 답 |
|---|---|
| 평범한 사진도 쓸 수 있는가 | 추출 후 주체의 크기, 위치, 방향, 크롭을 재설계합니다. |
| 원물을 알아볼 수 있는가 | 대표적인 윤곽, 흐름, 자세, 시각 기억점을 보존합니다. |
| 여백은 덜 그린 공간인가 | 그린 면적보다 분명히 크며 거리와 양·음의 형태를 적극 구성합니다. |
| 다양한 크기로 출력할 수 있는가 | 일반 비율, 정확한 픽셀, 네 모드, 폴더 일괄 처리를 지원합니다. |
...
- `design-only`: 전체 캔버스에 Panel 117의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
